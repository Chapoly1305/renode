...
PLOG=/tmp/pairing-flip.log; RLOG=/tmp/renode-15.4.log; ALOG=/tmp/otbr-diag-agent.log
SP="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"   # dir of this script (holds air_diag_sniff.py + mdns_query.py)
DIAG_DONE=0

AGENT_PID=""; RENODE_PID=""; SAMPLER_PID=""; AIRPID=""; RTT_PID=""
cleanup(){ kill -9 $AGENT_PID $RENODE_PID $SAMPLER_PID $AIRPID $RTT_PID 2>/dev/null; }
trap cleanup EXIT
...
# CASE disambiguation: wall-clock sniffer of the OT-sim air (bridge<->otbr). Direction by
# source port (9002=device->host, 9003=otbr host->device); large unicast DATA bursts = CASE
# fragments. Timestamps are unix-epoch, directly comparable to chip-tool's Sigma1 log time.
python3 "$SP/air_diag_sniff.py" 3600 > /tmp/air-diag-sniff.log 2>&1 &
AIRPID=$!
...
for attempt in $(seq 1 8); do
  ...
  setsid bash -c "timeout 900 ./renode --disable-xwt --hide-log --port 3456 $SCENARIO >/tmp/renode-stdout.log 2>&1" &
  RENODE_PID=$!
  gatt=0
  ...
  sleep 3
  # optional: drain the device SEGGER-RTT console (Matter/OpenThread logs) to /tmp/rtt.log via the monitor,
  # to see the device's own view of operational CASE (does it receive Sigma1 / try to send Sigma2?).
  if [ "${RTT_DRAIN:-0}" = 1 ]; then
    kill -9 $RTT_PID 2>/dev/null
    python3 "$SP/rtt_drain_monitor.py" 300 3456 >/tmp/rtt-drain.out 2>&1 &
    RTT_PID=$!
    echo "      [RTT drain started -> /tmp/rtt.log]"
  fi
  ...
  while kill -0 "$CHIP_PID" 2>/dev/null; do
    ...
    if [ "$conv_logged" = 0 ]; then
      ...
      if [ "$s" -gt 0 ]; then
        ...
        if [ "$DIAG_DONE" = 0 ]; then
          DIAG_DONE=1
          DIAGFILE=/tmp/diag-converged.txt
          {
            echo "############ CONVERGED-STATE DIAGNOSTIC @ $(printf '%(%H:%M:%S)T' -1) ############"
            echo "== chip-tool: exact operational instance names it is looking up =="
            grep -aoE "Lookup started for [0-9A-Fa-f-]+" "$PLOG" | sort -u | tail -5
            echo "== ot-ctl srp server service (device's _matter registration) =="
            "$OT_CTL" srp server service
            echo "== ot-ctl srp server host (registered AAAA) =="
            "$OT_CTL" srp server host
            echo "== ot-ctl netdata show =="
            "$OT_CTL" netdata show
            echo "== ephemeral QU mDNS query: _matter._tcp.local =="
            python3 "$SP/mdns_query.py" 6 _matter._tcp.local
            QN=$(grep -aoE "Lookup started for [0-9A-Fa-f-]+" "$PLOG" | tail -1 | awk '{print $NF}')
            echo "== chip-tool wants instance: '$QN' =="
            if [ -n "$QN" ]; then
              echo "== ephemeral QU mDNS query for chip-tool's EXACT instance =="
              python3 "$SP/mdns_query.py" 8 "${QN}._matter._tcp.local"
            fi
          } > "$DIAGFILE" 2>&1
          cp "$ALOG" /tmp/otbr-diag-agent-converged.log 2>/dev/null
          echo "      [diag captured to $DIAGFILE + /tmp/otbr-diag-agent-converged.log]"
        fi
  ...
  cp "$RLOG" "/tmp/radio-attempt-$attempt.log" 2>/dev/null   # M0: preserve this attempt's full radio-Info log before the next boot truncates $RLOG

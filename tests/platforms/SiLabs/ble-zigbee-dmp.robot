*** Variables ***
# The ELF variable must be set from command line to the DMP light elf file to be used. 
# For example: robot-test --variable ELF:path_to_elf_file/DynamicMultiprotocolLight.out this_test.robot
# RENODE-36: this app must be built without the "zigbee_display" component, this is the generation line:
# sled slc generate -s /path_to_gsdk/ -p /path_to_gsdk/protocol/zigbee/app/framework/scenarios/multiprotocol/DynamicMultiprotocolLight/DynamicMultiprotocolLight.slcp -d /path_to_destinaton/ --with brd4186c,zigbee_network_steering,iostream_usart:vcom,bluetooth_on_demand_start --without zigbee_display --configuration EMBER_AF_PLUGIN_CONCENTRATOR_CONCENTRATOR_TYPE:HIGH_RAM_CONCENTRATOR
